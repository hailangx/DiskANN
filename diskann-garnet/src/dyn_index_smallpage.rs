...
//! Approach I: Small-page unlimited post-filter.
//!
//! Like Approach E (unlimited effort) but uses a SMALL page size (L=200)
//! instead of the full effectiveEF (which can be 10K-50K). This means:
//!   - Each page graph traversal is very fast (~2ms vs ~400ms)
//!   - More pages are needed, but each is cheap
//!   - The search frontier is smaller, exploring more locally and incrementally
//!   - Total graph work is similar but per-page overhead is drastically reduced
//!
//! The hypothesis: DiskANN's `next_search_results` with L=10000 does a single
//! massive graph expansion that's slow. With L=200, each page is a small
//! incremental expansion that's cache-friendly and fast.
...
impl<T: VectorRepr> DynIndex for DiskANNIndex<GarnetProvider<T>> {
    ...
    /// Approach I: Small-page unlimited post-filter.
    ///
    /// Uses a SMALL page size (L=200) for fast per-page graph traversal,
    /// with unlimited effort to keep fetching pages until k results found.
    fn search_vector_filtered(
        &self,
        context: &Context,
        data: &[u8],
        ...
        // Use a small fixed L for fast per-page graph traversal.
        // The original l_value (effectiveEF) can be 10K-50K which makes each
        // page very expensive. With L=200, each page is ~2ms.
        let page_l = 200;

        // Always use FullPrecision — no BetaFilter callback overhead during traversal.
        let mut state = self.start_paged_search(FullPrecision, context, query, page_l)?;
        paged_filter_loop_unlimited(self, context, &mut state, k, page_l, filter_callback, output)
    }
        ...
        for candidate in &batch[..count] {
            ...
            if let Ok(eid) = eid_result {
                if output.push(eid, candidate.distance).is_full() || output.current_len() >= k {
                    ...
                    eprintln!(
                        "[paged-unlimited] pages={pages} evaluated={total_evaluated} passed={total_passed} found={} total={total_us}µs graph={}µs filter={}µs extid={}µs",
